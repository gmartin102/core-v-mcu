...
module dsp_type1_bw #(
  ...
  //assign s_clk = scan_mode_i ? scan_clk_i : clock_i;
...
`ifdef USE_DW
  wire [NBITS_A:0] a_in;
  wire [NBITS_B:0] b_in;
  wire [NBITS_A+NBITS_B+1:0] mult_o;

  assign a_in = unsigned_a ? {1'b0, mult_a} : {mult_a[NBITS_A-1], mult_a};
  assign b_in = unsigned_b ? {1'b0, mult_b} : {mult_b[NBITS_B-1], mult_b};
  assign mult = mult_o[NBITS_A+NBITS_B-1:0];
...
`endif
  ...
  always_comb begin
    ...
    case (feedback)
      ...
      1: add_b = 'h0;
      default: begin
        if (acc_fir < 6'd44) add_b = acc_fir_int << acc_fir;
        else add_b = acc_fir_int << 6'd44;
      end
    ...
    if ((|shift[NBITS_ACC-1:NBITS_Z-1] == 1'b0) || (&shift[NBITS_ACC-1:NBITS_Z-1] == 1'b1)) begin
      signed_saturate = {{(NBITS_ACC - NBITS_Z) {1'b0}}, {shift[NBITS_Z-1:0]}};
    end else begin
      signed_saturate = {
        {(NBITS_ACC - NBITS_Z) {1'b0}}, {shift[NBITS_ACC-1], {NBITS_Z - 1{~shift[NBITS_ACC-1]}}}
      };
    end

    saturate = sat_d2 ? (unsigned_mode ? unsigned_saturate : signed_saturate) : shift;
    z3 = saturate;
  end
