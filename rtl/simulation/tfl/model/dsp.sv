...
module dsp #(
    ...
    input       greset_ni,      // Global Reset Active Low
    input logic scan_reset_ni,  // Scan reset Active Low
  ...
  assign s_reset = scan_mode_i ? ~scan_reset_ni : ~greset_ni | reset_i;
  ...
  assign z_o = cfg_done_i ? (f_mode_i ? {dspf1_z, dspf0_z} : dsp0_z) : {NBITS_Z{1'b0}};

  assign dly_b_o = cfg_done_i ? (f_mode_i ? {dspf1_dly_b, dspf0_dly_b} : dsp0_dly_b):
                    {NBITS_B{1'b0}};
